...
export default function RankReveal({ entries, limit, valueLabel, resetKey }) {
  ...
  return (
    <div>
      <div className="rank-list">
        {shown.map((e) => (
          <div className="rank-row pop-in" key={e.rank}>
      ...
      {rest.length > 0 && revealedCount >= top.length && (
        <div style={{ marginTop: 16 }}>
          <p className="section-label" style={{ marginBottom: 8 }}>Reste du classement</p>
          <div className="rank-list">
            {rest.map((e) => (
              <div className="rank-row" key={e.rank} style={{ opacity: 0.6 }}>
                <span className="rank-number">{e.rank}</span>
                <span style={{ flex: 1, textAlign: 'left' }}>{e.pseudo}</span>
                <span className="score">{e.value} {valueLabel}</span>
              </div>
